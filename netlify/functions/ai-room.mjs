...
// Free-tier models on OpenRouter share a congested public pool and are sometimes deprecated
// with little notice, so we give OpenRouter an ordered fallback list (its "models" array) —
// if the first model is rate-limited or unavailable, it automatically tries the next one.
const MODEL_TEXT_CANDIDATES = (
      process.env.OPENROUTER_MODEL_TEXT ||
      "nex-agi/nex-n2.5-mini:free,qwen/qwen3.8-27b:free,nex-agi/nex-n2.5-pro:free"
    )
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const MODEL_VISION_CANDIDATES = (
      process.env.OPENROUTER_MODEL_VISION ||
      "nex-agi/nex-n2.5-mini:free,inclusionai/ling-3.0-flash-vl:free,qwen/qwen3.8-27b:free"
    )
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
export default async (req, context) => {
      ...
      // ---- Build the OpenRouter request (OpenAI-compatible chat completions API) ----
      let orBody;
      if (photoBase64) {
              // Vision models don't reliably support function/tool calling alongside images,
        ...
        orBody = {
                  models: MODEL_VISION_CANDIDATES,
                  max_tokens: 400,
      ...
      try {
              orRes = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                        ...
                        headers: {
                                    ...
                                    "HTTP-Referer": "https://abyld.com",
                                    "X-Title": "Abyld AI Room Demo",
                        },
                        body: JSON.stringify(orBody),
              });
      } catch (err) {
              console.error("ai-room: fetch to OpenRouter failed", err);
              return jsonResponse(502, { error: "Couldn't reach the AI service. Try again." });
      }

      if (!orRes.ok) {
              const errText = await orRes.text();
              console.error("ai-room: OpenRouter API error", orRes.status, errText);
